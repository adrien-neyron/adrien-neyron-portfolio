...
async function handleSubmit(payload: Record<string, unknown>) {
  ...
  try {
    ...
    await $fetch("/api/admin/ai-tools", {
      method:  "POST",
    ...
    error.value = "Erreur lors de la création de l'outil.";
    loading.value = false;
  }
}
</script>
...
<template>
  <div class="p-8 max-w-4xl">
    <div class="mb-8">
      ...
      <h1 class="text-2xl font-bold mt-2" style="font-family: var(--font-title);">Nouvel outil IA</h1>
    </div>
    ...
    <div class="p-6 rounded-xl border" style="border-color: color-mix(in srgb, var(--color-accent) 15%, transparent); background-color: var(--color-surface);">
      <AdminAiToolForm :loading="loading" @submit="handleSubmit" />
    </div>
  </div>
</template>
